...
@records_bp.route("/records/<rid>/<ts>")
def record_files_page(rid, ts):
    """单条记录的文件清单页（「逐个下载文件」跳到这里）。

    刻意做成独立页面，而不是塞在首页里的一个展开区：
    首页那个展开区一旦展开就只能整块关掉、收不回去，而且文件清单是动态覆写
    首页 DOM 的，容易和首页自己的状态互相干扰。独立页面还有一个好处 ——
    可以直接分享/收藏某条记录的清单地址。

    这里不校验记录是否存在：页面自己会去调 /api/records/<rid>/<ts> 并显示错误，
    省一次重复的目录探测。
    """
    return send_from_directory(HERE, "record_files.html")
